...
func Load(configPath string) (*Config, error) {
	// 加载环境变量
	_ = godotenv.Load(filepath.Join(ProjectRootPath, ".env"))
...
// ===== 工具函数:获取配置文件路径(项目根目录+不同环境的config文件名称) =====
func getDefaultConfigPath() string {
	env := os.Getenv("APP_ENV")
	if env == "" {
		env = "local"
	}
	return filepath.Join(ProjectRootPath, fmt.Sprintf("common/config/config.%s.yaml", env))
}
